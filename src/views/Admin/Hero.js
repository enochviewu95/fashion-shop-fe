...
export default function Hero({ pageTitle }) {
  ...
  return (
    <div>
      <div className="my-5">
      <Link to="/fashion-shop-fe/admin/home/add-hero" className={`${buttonBackground} px-3 py-2 hover:${buttonHoverBackground} text-white my-5 rounded-lg shadow-sm`}>Add Banner</Link>
      </div>
      {banners
        ? banners.map(banner => <HeroSection key={banner._id} banner={banner} />)
        : ""}
    </div>
  );
}
